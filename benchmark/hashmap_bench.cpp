...
static size_t capacity_for_elements(size_t elements) {
    return (((elements * 100) + HASHMAP_LOAD_FACTOR_PERCENT - 1) / HASHMAP_LOAD_FACTOR_PERCENT) + 1;
}
...
template <typename Map> static void BM_Std_InsertReserved(benchmark::State& state) {
    const auto count = static_cast<size_t>(state.range(0));
    const auto keys = make_present_keys(count);
...
template <typename Map> static void BM_Std_InsertGrowing(benchmark::State& state) {
    const auto count = static_cast<size_t>(state.range(0));
    const auto keys = make_present_keys(count);
...
static void accumulate_stats(HashMapStats* totals, const HashMapStats& stats) {
    ...
    totals->lookup_hits += stats.lookup_hits;
    totals->remove_calls += stats.remove_calls;
    totals->remove_probes += stats.remove_probes;
    totals->reserved_inserts += stats.reserved_inserts;
    totals->growing_inserts += stats.growing_inserts;
    totals->resize_count += stats.resize_count;
    totals->resize_only_ns += stats.resize_only_ns;
    totals->migrated_elements += stats.migrated_elements;
    totals->entries_reinserted += stats.entries_reinserted;
    totals->allocations += stats.allocations;
    totals->groups_scanned_during_reinsertion += stats.groups_scanned_during_reinsertion;
}

static void
BM_HashMap_Stats(benchmark::State& state, const HashMapStats& totals, double item_count) {

    const double iterations = static_cast<double>(state.iterations());
    state.counters["hashes/op"] = static_cast<double>(totals.hash_calculations) / iterations;
    state.counters["hashes/item"] = static_cast<double>(totals.hash_calculations) / item_count;
    ...
    state.counters["resizes/iteration"] = static_cast<double>(totals.resize_count) / iterations;
    ...
    state.counters["allocations/iteration"] = static_cast<double>(totals.allocations) / iterations;
    state.counters["groups-scanned-during-reinsertion/iteration"] =
        static_cast<double>(totals.groups_scanned_during_reinsertion) / iterations;
    state.counters["groups/item"] =
        static_cast<double>(totals.groups_scanned_during_reinsertion) / item_count;
}
#endif
...
static void BM_HashMap_LoadFactor_Hit(benchmark::State& state) {
    ...
    state.counters["actual_load"] = static_cast<double>(count) / static_cast<double>(capacity);
...
static void BM_HashMap_ConfiguredLoadFactor_Insert_Stats(benchmark::State& state) {
    const auto load_factor = static_cast<size_t>(state.range(0));
    const auto count = static_cast<size_t>(1U << 14U);
    const auto keys = make_present_keys(count);
    HashMapStats totals{};
...
static void BM_HashMap_Rehash(benchmark::State& state) {
    ...
    state.counters["hashes/item"] = static_cast<double>(totals.hash_calculations) / items;
    state.counters["groups/item"] =
...
static void BM_HashMap_Stats_inner(benchmark::State& state, MapInitializer init_map_fn) {
    const auto count = static_cast<size_t>(state.range(0));
    const auto present = make_present_keys(count);
    const auto missing = make_missing_keys(count);
    HashMapStats totals{};
    ...
    BM_HashMap_Stats(
        state,
        totals,
        static_cast<double>(state.iterations()) * static_cast<double>(count) * 3.0
    );
    state.SetItemsProcessed(static_cast<int64_t>(state.iterations()) * static_cast<int64_t>(count));
}
...
static void BM_HashMap_InsertAtOccupancy_Stats(benchmark::State& state) {
    ...
    state.counters["hashes/op"] =
        static_cast<double>(totals.hash_calculations) / static_cast<double>(totals.put_calls);

    state.counters["insert-probes/put"] =
        static_cast<double>(totals.insert_probes) / static_cast<double>(totals.put_calls);

    state.counters["swaps/put"] =
        static_cast<double>(totals.insert_swaps) / static_cast<double>(totals.put_calls);

    state.counters["resizes/iteration"] =
        static_cast<double>(totals.resize_count) / static_cast<double>(state.iterations());

    state.SetItemsProcessed(
        static_cast<int64_t>(state.iterations()) * static_cast<int64_t>(insert_count)
    );
}
#endif
...
BENCHMARK(BM_HashMap_InsertAtOccupancy_Stats)->Apply(LoadFactors)->Iterations(1000);
#endif
