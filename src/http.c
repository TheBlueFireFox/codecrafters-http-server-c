...
static size_t write_endline(uint8_t* const buf) {
    STRVAL(buf, ENDLINE);
}
...
size_t write_headers(uint8_t* const buf, HttpHeaders* headers) {
    ...
    for (vector_each(header, &headers->headers)) {
        size += sprintf((char*)buf + size, "%s: %s" ENDLINE, header->key, header->value);
    }
...
static size_t parse_headers(uint8_t* buf, HttpHeaders* headers) {
    ...
    if (content_encoding != NULL && strstr(content_encoding, GZIP_ENCODING) != NULL) {
...
static size_t
parse_request_line(uint8_t* buf, HttpMethod* method, HttpVersion* version, const char** url) {
    // GET                          // HTTP method
...
HttpRequest parse_request(uint8_t* buf) {
    ...
    headers.connection.active =
        ((connection_state == NULL || strcmp(connection_state, CONNECTION_CLOSE) != 0) != 0);
...
void free_http_request(HttpRequest* req) {
    vector_free(&req->headers.headers);
}
...
HttpResponse init_response(
    ...
    HttpHeaders headers = {.encoding = encoding,
