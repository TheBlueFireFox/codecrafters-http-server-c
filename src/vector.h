...
// the vector
void vector_init_with_capacity_impl(VectorInternal* vec, size_t capacity, size_t obj_size);

#define vector_init_with_capacity(vector, capacity)                                                \
    vector_init_with_capacity_impl((&(vector)->internal), capacity, vector_payload_size(vector));
...
#define vector_push(vector, item)                                                                  \
    vector_push_impl(                                                                              \
        (&(vector)->internal),                                                                     \
        (1 ? &(item) : ((vector)->payload)),                                                       \
        vector_payload_size(vector)                                                                \
    )

bool vector_get_impl(VectorInternal* vec, size_t idx, uint8_t* val, size_t obj_size);
...
void* vector_search_impl(VectorInternal* vec, size_t obj_size, const void* key, compFn comp_fn);
