...
static void resize_queue(QueueInternal* queue, size_t obj_size) {
    ...
    memcpy(queue->buffer + (old_capacity * obj_size), queue->buffer, elem_at_start * obj_size);
...
void queue_enqueue_impl(QueueInternal* queue, uint8_t const* const val, size_t obj_size) {
