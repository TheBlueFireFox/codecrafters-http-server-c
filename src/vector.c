...
void vector_push_impl(VectorInternal* vec, void const* const elem, size_t obj_size) {
...
bool vector_get_impl(VectorInternal* vec, size_t idx, uint8_t* const val, size_t obj_size) {
    if (vec->len <= idx) {
        return false;
    }
