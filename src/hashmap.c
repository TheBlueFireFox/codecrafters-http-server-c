...
HashMapSlotConfigurations hashmap_slot_config(struct HashMapSlotQuery* query) {
    ...
    size_t key_offset = 0;
    size_t value_offset = align_up(key_offset + query->key_size, query->value_alignment);
...
static inline HashMapSlot hashmap_get_slot_inner(HashMapInternal* map, size_t idx, uint8_t* data) {
    // struct HashMapSlot {
...
static inline HashMapSlot hashmap_get_slot_impl(HashMapInternal* map, size_t idx) {
...
static void hashmap_init_internal_impl(
    HashMapInternal* map,
    ...
    map->control = hashmap_setup_region(HASHMAP_DEFAULT_CAPACITY, HASHMAP_HASH_EMPTY);
...
void hashmap_clear_impl(HashMapInternal* map) {
    map->len = 0;
    memset(map->control, HASHMAP_HASH_EMPTY, map->capacity + HASHMAP_PROBE_GROUP_SIZE);
}

static inline uint64_t hashmap_mod_capacity(HashMapInternal* map, uint64_t value) {
...
static inline uint64_t hashmap_group_match(uint64_t group, uint8_t fingerprint) {
    ...
    uint64_t matching_groups = (x - HASHMAP_NEEDLE_MAP) & ~x & HASHMAP_MATCHES_MAP;
    return matching_groups;
}

static void hashmap_control_set(HashMapInternal* map, size_t idx, uint8_t h2, uint8_t distance) {
...
static bool hashmap_put_inner_displaced(
    HashMapInternal* map,
    const void* key,
    const void* value,
    size_t idx,
...
static inline bool hashmap_put_inner(
    HashMapInternal* map,
    Hash hash,
    const void* key,
    const void* value,
    ...
    while (true) {
        ...
        if (distance > slot_distance) {
            return hashmap_put_inner_displaced(map, key, value, idx, h2, distance, reinserting);
        }
...
static void hashmap_resize(HashMapInternal* map, size_t new_capacity) {
    ...
    int64_t elapsed_ns = ((int64_t)(resize_end.tv_sec - resize_start.tv_sec) * 1000000000LL) +
...
bool hashmap_remove_impl(HashMapInternal* map, const void* key) {
    ...
    while (true) {
        ...
        hashmap_control_set(map, idx, *(map->control + next_idx), *(map->distance + next_idx) - 1);
