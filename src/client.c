...
static size_t load_request(int client_fd, uint8_t** in_buf, size_t* buffer_size, HttpRequest* req) {
    size_t read_size = 0;
...
static bool handle_client_request(
    int client_fd,
    uint8_t** in_buf,
    uint8_t* out_buf,
    size_t* buffer_size,
...
static enum LoopInnerState
handle_client_loop_inner(struct pollfd* fds, size_t fds_count, size_t* iter_count) {
    *iter_count -= INTERVAL;
...
static void handle_client_loop(
    int client_fd,
    uint8_t** in_buf,
    uint8_t* out_buf,
...
void handle_client(int client_fd, AppState* state, atomic_bool* server_running) {
    ...
    handle_client_loop(client_fd, &in_buf, out_buf, (size_t)INITIAL_BUFFER, state, server_running);
