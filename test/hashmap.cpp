...
void hashmap_first_byte_algo(void* config) {
    (void)config;
}
...
TEST_P(HashMapAlignmentTest, ProducesCorrectlyAlignedSlots) {
    ...
    for (size_t i = 0; i < capacity; ++i) {
        ...
        EXPECT_EQ(reinterpret_cast<uintptr_t>(slot + config.key_offset) % p.key_align, 0);

        EXPECT_EQ(reinterpret_cast<uintptr_t>(slot + config.value_offset) % p.value_align, 0);
    }
...
INSTANTIATE_TEST_SUITE_P(
    HashMapSlotLayouts,
    HashMapAlignmentTest,
    ::testing::Values(
        AlignmentCase{8, 8, 1, 1, 8, 8},
...
TEST(TestHashMap, firstByteAlgo) {
    ...
    hashmap_init_with_algo(&map, FirstByteAlgo, &hashmap_hash_i32, &hashmap_equal_bytes);
...
class TestHashersU8 : public ::testing::TestWithParam<std::tuple<uint8_t, Hash>> {};
...
INSTANTIATE_TEST_SUITE_P(
    ...
    TestHashersU8,
    ::testing::Values(
        std::make_tuple(0x01, 0xaf63bc4c8601b62c),
...
class TestHashersU64le : public ::testing::TestWithParam<std::tuple<uint64_t, Hash>> {
  protected:
...
INSTANTIATE_TEST_SUITE_P(
    ...
    TestHashersU64le,
    ::testing::Values(
        std::make_tuple(0x0000000000000000, 0xa8c7f832281a39c5),
...
TEST(TestHashMap, initFNV1a) {
    ...
    for (size_t i = 0; i < map.internal.capacity + HASHMAP_PROBE_GROUP_SIZE; i += 1) {
...
TEST(TestHashMap, getSlotByIndex) {
    ...
    EXPECT_EQ(static_cast<uint8_t*>(slot.key), slot_location + map.internal.key_offset);
    EXPECT_EQ(static_cast<uint8_t*>(slot.value), slot_location + map.internal.value_offset);
...
TEST(TestHashMap, putFirstByteAlgo) {
    ...
    EXPECT_EQ(*(map.internal.control + map.internal.capacity + key), HASHMAP_HASH_H2(hash));
...
TEST(TestHashMap, putMultiple) {
    ...
    for (size_t i = 0; i < 4; i += 1) {
        ...
        EXPECT_EQ(*(map.internal.control + idx), expected_keys[i]) << " idx " << idx << " i " << i;
        EXPECT_EQ(*(map.internal.control + map.internal.capacity + idx), expected_keys[i])
            << " idx " << idx << " i " << i;
        EXPECT_EQ(*(map.internal.distance + idx), expected_distance[i])
            << " idx " << idx << " i " << i;
        EXPECT_EQ(*(map.internal.distance + map.internal.capacity + idx), expected_distance[i]);

        EXPECT_EQ(*(int*)slot.key, expected_keys[i]) << " idx " << idx << " i " << i;
        EXPECT_STREQ(*(const char**)slot.value, expected_values[i]) << " idx " << idx << " i " << i;
    }
...
TEST(TestHashMap, fillToCapacity) {
    ...
    const size_t entries_before_resize = ((initial_capacity * HASHMAP_LOAD_FACTOR_PERCENT) / 100);
...
void assert_map_matches_reference(
    IntIntHashMapTest* map,
    const std::unordered_map<int, int>& reference
) {
...
INSTANTIATE_TEST_SUITE_P(
    Seeds,
    HashMapRandomizedTest,
    ::testing::Values(0U, 1U, 42U, 0xC0FFEEU, 0xDEADBEEFU, 0xFFFFFFFFU)
);
...
TEST_P(HashMapRandomizedTest, MatchesStdUnorderedMap) {
    ...
    for (int operation_index = 0; operation_index < kOperations; ++operation_index) {
        ...
        switch (operation) {
            ...
            case 2: {
                ...
                if (expected == reference.end()) {
                    EXPECT_EQ(actual, nullptr) << "operation=" << operation_index << " key=" << key;
                } else {
                    ASSERT_NE(actual, nullptr) << "operation=" << operation_index << " key=" << key;
    ...
    for (int i = 0; i < kOperations; ++i) {
        ...
        switch (operation) {
            ...
            case 1: {
                ...
                EXPECT_EQ(actual_removed, expected_removed) << "iteration=" << i << " key=" << key;
        ...
        for (const auto& [expected_key, expected_value] : reference) {
            ...
            ASSERT_NE(actual, nullptr) << "iteration=" << i << " key=" << expected_key;

            EXPECT_EQ(*actual, expected_value) << "iteration=" << i << " key=" << expected_key;
        }
    }
...
TEST_P(HashMapRandomizedTest, SurvivesRepeatedResizes) {
    ...
    for (int key = 0; key < kOperations; ++key) {
        ...
        for (const auto& [expected_key, expected_value] : reference) {
            ...
            ASSERT_NE(actual, nullptr) << "inserted_key=" << key << " missing_key=" << expected_key;
